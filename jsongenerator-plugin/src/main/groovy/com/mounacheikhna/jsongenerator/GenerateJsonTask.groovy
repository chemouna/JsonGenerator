...
public class GenerateJsonTask extends DefaultTask implements GenerateJsonSpec {
    ...
    //TODO: make the use of first pass and second pass more general and can accept any number of nested steps
    ...
    @Override
    void firstPassConfig(String firstPassConfigPath, String startKeyDelimiter, String endKeyDelimiter, boolean escaped) {
        firstPassConfig = new PassConfig(firstPassConfigPath, startKeyDelimiter, endKeyDelimiter, escaped)
    }

    @Override
    void secondPassConfig(String secondPassConfigPath, String startKeyDelimiter, String endKeyDelimiter, boolean escaped) {
        secondPassConfig = new PassConfig(secondPassConfigPath, startKeyDelimiter, endKeyDelimiter, escaped)
    }
    ...
    @Override
    void jsonsMappings(Map<String, String> mappings) {
        this.jsonsMappings = mappings
    }
    ...
    class JsonConfig {
        ...
        JsonConfig(String locale, String productFlavor, PassConfig firstPassConfig, PassConfig secondPassConfig) {
            this.locale = locale
        ...
        File createJson(placeholderFileName, jsonFileName) {
            println "createJson from placeholder $placeholderFileName for json $jsonFileName"
            def placeholderFile = new File("${baseFolder}/$placeholderFileName")
            File file = new File("${baseFolder}/$jsonFileName")
            file = fillPlaceholders(placeholderFile, file, firstPassConfig)
            return fillPlaceholders(placeholderFile, file, secondPassConfig)
        }

        File fillPlaceholders(File inputFile, File outputFile, PassConfig passConfig) {
            def inputFileText = inputFile.getText('UTF-8')
            passConfig.extractedValues.each {
                key, value ->
                    if (passConfig.escaped) {
                        inputFileText = inputFileText.replaceAll("\\${passConfig.startKeyDelimiter}$key\\${passConfig.endKeyDelimiter}", value.toString())
                    } else {
                        inputFileText = inputFileText.replaceAll("${passConfig.startKeyDelimiter}$key${passConfig.endKeyDelimiter}", value.toString())
                    }
            }
    ...
    class PassConfig {
        ...
        boolean validate() {
            if (filePath?.trim() && !new File(filePath).exists()) {
                throw new IllegalArgumentException("Please provide a correct path to config file : $filePath ")
            }
            return true
        }
    }
